...
В [**tutorial part zero**](/tutorial/part-zero/) мы создали новый сайт на основе "hello world" стартера, с помощью команды:
...
![01-hello-world-vscode](01-hello-world-vscode.png)

_Заметка: в примере показан редактор Visual Studio Code. В случае использования другого редактора, он может выглядеть немного иначе (😱 да ладно)_

Посмотрим на код, который приводит в действие домашнюю страницу.

> 💡 Если ты остановил сервер разработки после запуска `gatsby development` в последний раз, то запусти его снова ― самое время внести некоторые изменения в сайт hello-world!
...
![03-more-hot-reloading](03-more-hot-reloading.png)
...
![04-add-image](04-add-image.png)

### Погодите... HTML в JavaScript?

> Если ты знаком с React и JSX, то смело пропускай этот раздел. Если ты ещё не работал с React, то возможно тебе будет интересно, что делает HTML в коде JavaScript, или почему мы импортируем `'react'` в первой строке, но нигде не используем его. Этот гибридный "HTML-in-JS" на самом деле является синтаксическим расширением JavaScript для React, под называнием JSX. Можно продолжить изучать это руководство и без предварительного опыта работы с React, но если тебе всё же интересно, то вот краткая инструкция...
...
Одной из крышесносных открытий, которое ты сделаешь, когда начнешь работать с компонентами, это то, что теперь CSS, HTML и JavaScript будут находиться очень близко. Порой настолько близко, что будут располагаться все в одном файле.

Это, казалось бы, простое изменение имеет серьезные последствия в том, как в построении мышления при создании веб-сайтов.
...
![05-about-page](05-about-page.png)
...
![06-header-component](06-header-component.png)

Теперь текст заголовка на странице "About Gatsby" должен быть заменен на "This is a header.", но нам это не подходит, мы хотим, чтобы на странице **about** говорилось "About Gatsby".
...
> 💡 В JSX можно встроить любое выражение JavaScript, заключив его в `{}`. Вот как мы можем получить доступ к свойству `headerText` из объекта "props".
...
![08-duplicate-header](08-duplicate-header.png)

Иии... у нас получилось; Второй заголовок без переписывания кода, путем передачи различных данных через "props".
...
Когда ты нажимаешь на новую ссылку "Contact" на главной странице, то должен увидеть...

![09-dev-404](09-dev-404.png)
...
Если ты ранее не устанавливал &amp; Surge, то открой новое окно терминала и установи его с помощью командной строки:
...
Сборка должна занять 15-30 секунд. По завершению, нам интересно взглянуть на файлы, которые команда `gatsby build` только что подготовила для развертывания.

Посмотрим список сгенерированных файлов, введя следующую команду терминала в корне проекта, которая позволит взглянуть на каталог `public`:
...
По окончанию, ты должен увидеть в своем терминале что-то вроде:

![surge-deployment](surge-deployment.png)
...
- узнал о компонентах
- узнал о компонентах страниц и подкомпонентах Gatsby
- узнал о "props" и переиспользуемых компонентах в React
