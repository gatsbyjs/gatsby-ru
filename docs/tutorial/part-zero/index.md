...
1. Откройте терминал.
2. Запустите `gatsby new hello-world https://github.com/gatsbyjs/gatsby-starter-hello-world`. (_Заметка: Время создания проекта зависит от скорости инетрнет подключения. Для экономии времени, пример ниже был приостановлен во время установки._)
...
**Заметка:** Если вы пользуетесь виртуальной машиной, например, `vagrant` и/или хотите прослушивать локальный IP-адрес, то следует запускать сервер с дополнительными флагами `gatsby develop --host=0.0.0.0`, чтобы сервер разработки находился на "localhost" и локальном IP-адресе.
