...
Плагины Gatsby это JavaScript пакеты которые помогают добавлять функциональность Gatsby сайту. Gatsby спроектирован расширяемым, это означает что плагины могут расширять и изменять практически все что Gatsby делает.
...
Участники сообщества (как вы!) могут принять участие в создании плагинов (небольшое количество JavaScript кода) которые могут быть использованы другими пользователями при создании Gatsby сайтов.
...
Как уже отмечалось во [второй части](/tutorial/part-two/), на данном этапе, возможно неплохой идеей будет закрыть окно терминала и файлы проекта с предыдущей части этого руководства, чтобы начать все с чистого листа. Далее, откройте новое окно терминала и выполните следующие комманды для того чтобы создать новый сайт Gatsby в папке под названием `tutorial-part-three` и далее перейти в неё:
...
Два главных этапа использования этого плагина это инсталяция и настройка.
...
> Примечание: Typography.js требует несколько дополнительных пакетов, поэтому они включены в команду установки. Такие дополнительные требования будут перечисленны в команде установки каждого плагина.
...
`gatsby-config.js` это еще один специальный файл который Gatsby автоматически распознает. В этом файле обычно содержатся плагины и другие настройки сайта.

> Взгялните на [документацию по gatsby-config.js](/docs/gatsby-config/) чтобы узнать больше.
...
Как только сайт загрузится используйте средства разработки Chrome чтобы проинпектировать сгенерированный HTML, вы увидите что плагин типографики добавил `<style>` элемент с сгенерированным CSS в `<head>` элемент:

![typography-styles](typography-styles.png)

### ✋ Внесение изменений в содержимое и стили

Скопируйте следующее содержимое в ваш `src/pages/index.js` файл чтобы вы могли увидеть эффект от применения CSS стилей сгенерированых библиотекой Typography.js.
...
Давайте взгялнем на то как выглядит страница:

![about-uncentered](about-uncentered.png)

Хмм. Было бы хорошо если бы содержимое двух новых страниц было отцентрированно так же как на стартовой странице. И было бы еще лучше использовать глобальную навигацию для того чтобы посетителям было легче находить и посещять каждую из подстраниц.

Вы решите эту проблему создав ваш первый компонент разметки.

### ✋ Создание вышего первого компонента разметки

1. Создайте новую папку `src/components`.

2. Создайте очень просто компонент в этой папке `src/components/layout.js`:
...
Отлично, разметка работает! Содержимое вашей стратовой страницы отцентрированно.

Но попробуйте перейти на страницу `/about/`, или `/contact/`. Содержимое данных страниц все еще не отцентрированно.
...
Содержимое всех трех страниц теперь отцентрированно благодаря одному компоненту разметки!
...
Перейдем в [четвертую часть урока](/tutorial/part-four/) где вы начнете изучать слой данных Gatsby и программно создавать Gatsby страницы!
